...
$stmt = $bdd->prepare("SELECT ID_Promotion FROM etudiant WHERE ID_utilisateur = ?");
$stmt->execute(array($user));
$rtrn=$stmt->fetch();
if($rtrn){
    $ID_Promotion=$rtrn["ID_Promotion"];
    $stmt = $bdd->prepare("SELECT Nom FROM promotion WHERE ID = ?");
    $stmt->execute(array($ID_Promotion));
    $rtrn=$stmt->fetch();
    ...
    echo "aucun étudiant ne correspond";
}
...
<html>
    ...
    <body>
        ...
        ?>
            ...
            <div id='milieu'>
                ...
                <form method="POST" action="BDDmodifier.php?next=<?=$next?>" id="my-form">
                    ...
                    <p id="CasePromotion" class="large">
                        ...
                        <br>
                        <input type="text" id="Promotion" name="Promotion" required value="<?=$Promotion?>">
                    </p>
            ...
            <?php }else{ ?>
                <div class="large"> Vous n'avez pas les droits pour modifier un étudiant. </div>
                <br>
